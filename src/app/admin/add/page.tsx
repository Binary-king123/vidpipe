...
import { addAndProcessVideo } from '../actions'
...
export default function BulkDownloaderPage() {
  ...
  useEffect(() => {
    ...
    if (loading) {
      ...
      interval = setInterval(() => {
        setProgress(p => (p < 95 ? p + 2 : p)) // fake progress to 95%
      }, 500)
    } else {
      setProgress(100)
    }
  ...
  async function handleProcessNext() {
    ...
    fd.append('url', activeUrl)
    fd.append('title', title)
    fd.append('category', category)
    fd.append('tags', tags)
    fd.append('thumbTime', thumbTime)
    fd.append('thumbUrl', thumbUrl)
    fd.append('description', description)

    try {
      const res = await addAndProcessVideo(fd)
      if (res.success) {
        if (currentIdx + 1 < urls.length) {
          setCurrentIdx(i => i + 1)
          resetForm()
        } else {
          setPhase('done')
  ...
  return (
    <div className="p-8 max-w-3xl">
      ...
      <p className="text-gray-400 mb-8">
        Add multiple CDN URLs, preview thumbnails, set custom covers, and process them one-by-one with satisfying feedback.
      </p>
      ...
      {phase === 'input' && (
        <div className="bg-gray-900 border border-gray-800 p-6 rounded-xl shadow-lg">
          ...
          >
            Start Processing Processing Queue ({rawText.split('\n').filter(l => l.trim().startsWith('http')).length} videos)
          </button>
        </div>
      )}
          ...
          <div className="bg-gray-900 border border-gray-800 p-6 rounded-xl shadow-lg space-y-5">
            ...
            <div className="bg-gray-950 border border-gray-800 p-4 rounded-lg">
              ...
              <video 
                ...
                src={urls[currentIdx]} 
                controls 
                className="w-full h-auto max-h-64 bg-black rounded"
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-gray-950 border border-gray-800 rounded-lg px-4 py-2 text-white" disabled={loading}
                />
              </div>
              <div>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    value={thumbTime} onChange={e => setThumbTime(e.target.value)}
                    className="w-full bg-gray-950 border border-gray-800 rounded-lg px-4 py-2 text-white" disabled={loading}
                  />
                  <button
                    type="button"
                    onClick={() => { if (videoRef.current) setThumbTime(videoRef.current.currentTime.toFixed(1)) }}
                    className="bg-gray-800 hover:bg-gray-700 text-xs px-3 py-2 rounded-lg text-white"
                  >
                    Select Frame
                  </button>
                </div>
              ...
              {loading && (
                <div className="mb-4">
                  <div className="flex justify-between text-xs text-pink-400 mb-1">
                    <span>Downloading & Processing (FFmpeg)...</span>
                    <span>{progress}%</span>
                  </div>
                  <div className="w-full bg-gray-800 rounded-full h-2">
                    <div className="bg-pink-500 h-2 rounded-full transition-all duration-300" style={{ width: `${progress}%` }}></div>
                  </div>
                </div>
              )}
      ...
      {phase === 'done' && (
        <div className="bg-gray-900 border border-gray-800 p-12 rounded-xl text-center space-y-4">
          ...
          <p className="text-gray-400">The entire bulk queue has been successfully compressed and published.</p>
          <button
            onClick={() => { setPhase('input'); setUrls([]); setRawText(''); }}
