...
export default async function QueuePage() {
  ...
  return (
    <div className="p-8">
      ...
      <p className="text-gray-400 mb-8">Total: {total} | Pending: {pending} | Failed: {failed} (Showing last 50)</p>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
        <table className="w-full text-left text-sm text-gray-400">
          ...
          <tbody className="divide-y divide-gray-800">
            {queue.length === 0 ? (
              <tr><td colSpan={5} className="px-6 py-8 text-center text-gray-500">Queue is empty.</td></tr>
            ) : (
              queue.map((item) => (
                <tr key={item.id} className="hover:bg-gray-800/30 transition-colors">
                  <td className="px-6 py-4 font-medium text-gray-500">#{item.id}</td>
                  <td className="px-6 py-4 truncate max-w-[300px]" title={item.cdnUrl}>{item.cdnUrl}</td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">{item.createdAt.toLocaleString()}</td>
                </tr>
              ))
            )}
          </tbody>
