...
export default async function AdminDashboard() {
  ...
  const pending = queueCounts.find((q: any) => q.status === 'pending')?._count.id || 0
  const processing = queueCounts.find((q: any) => q.status === 'processing')?._count.id || 0
  const done = queueCounts.find((q: any) => q.status === 'done')?._count.id || 0
  const failed = queueCounts.find((q: any) => q.status === 'failed')?._count.id || 0
